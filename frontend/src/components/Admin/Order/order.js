...
export default function Order({ getApiOrder, order }) {
    ...
    const handleAcceptCancel = (orderID, status, accountID) => {
        ...
        axios.put(`http://localhost:8080/api/order/cancel`, formUpdate, {
            headers: {
                'Content-Type': 'application/json'
            }
        })
                        ...
                        return (
                            <Row className={`order-title w-full h-[88px] py-4 flex items-center ${index % 2 == 0 ? 'bg-white' : ''}`}>
                                ...
                                <Col className='text-center' xl={{ span: 2, offset: 0 }}>
                                    ...
                                    {item.statusID == 5 && <div className=''>
                                        <>
                                            <Button color='red' type="primary" onClick={() => handleOpenCancel(item.orderID)}>
                                                Duyệt yêu cầu
                                            </Button>
                                            <Modal footer={null} title="" open={isModalCancelOpen} onCancel={handleCancel}>
                                                <DialogHeader className='flex justify-center'>
                                                    HÓA ĐƠN THANH TOÁN
                                                </DialogHeader>
                                                <DialogBody>
                                                ...
                                                <DialogFooter className='flex justify-center'>
                                                    ...
                                                    <Button onClick={() => handleAcceptCancel(item.orderID, 6, item.accountID)} className='w-[200px] text-base' color='blue' size='lg' variant="filled">
                                                        Xác nhận
                                                    </Button>
                                                </DialogFooter>
                                            </Modal>
                                    ...
                                    {item.statusID == 6 && <Tag className='rounded-full w-full flex justify-center py-1 px-2 text-sm border-none' color="error">Đã hủy</Tag>}
                                </Col>
                            </Row>)
                    } else
                    ...
                    if (index >= (currentPage.page * currentPage.size - currentPage.size) && index < (currentPage.page * currentPage.size)) {
                        return (
                            <Row className={`order-title w-full h-[88px] py-4 flex items-center ${index % 2 == 0 ? 'bg-white' : ''}`}>
                                ...
                                <Col className='text-start' xl={{ span: 3, offset: 0 }}>#{item.orderID}</Col>
                                <Col className='text-start' xl={{ span: 4, offset: 0 }}>{item.name}</Col>
                                <Col className='text-start' xl={{ span: 7, offset: 0 }}>{`${item.city} - ${item.district} - ${item.address}`}</Col>
                                <Col className='text-center' xl={{ span: 4, offset: 0 }}>{formatDate(item.time)}</Col>
                                <Col className='text-center' xl={{ span: 3, offset: 0 }}>{formatNumber(item.total)}</Col>
